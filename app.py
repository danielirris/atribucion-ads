...
def _construir_filas(anuncios, nivel, insights, ventas_agg, ahora):
    ...
    def _ing_usd(por_moneda, fallback):
        """Convierte un desglose {moneda: total_nativo} a USD usando la tasa de CADA
        moneda. Las ventas cuya moneda quedó desconocida ('?') usan `fallback` (la
        moneda de la cuenta o la configurada). Cada venta se convierte con SU moneda."""
        tot = 0.0
        for mon, val in (por_moneda or {}).items():
            tot += val * _rate(fallback if mon == "?" else mon)
        return tot
    ...
    for gkey, g in grupos.items():
        ...
        if fuente_ventas == "meta":
            ...
            # Cada venta se convierte con SU moneda (país). moneda_v es el respaldo
            # para las ventas cuya moneda no se pudo determinar ('?').
            _pm = {}
            for x in ads:
                for mon, val in (ventas_agg.get(x["ad_id"], {}).get("por_moneda") or {}).items():
        ...
        if ab:
            ...
            imod_usd = _ing_usd(vs_mod.get("por_moneda"), moneda_v)
            ventas_mod = int(vs_mod.get("num", 0))
            # ROAS "desde el cambio" en USD (gasto e ingreso convertidos, cada uno con
            # su moneda) para que sea correcto aunque cuenta y ventas usen monedas distintas.
            gmod_usd = gmod_nat * rate_c
            roas_mod = (imod_usd / gmod_usd) if gmod_usd > 0 else 0.0
        else:
            inicio_mod, gmod_nat, imod_nat, imod_usd, roas_mod, ventas_mod = \
        ...
        for _d in _dias7:
            _pm_d = {}
            for aid in ad_ids_grupo:
                for mon, val in (_diario7.get(aid, {}).get(_d) or {}).items():
                    _pm_d[mon] = _pm_d.get(mon, 0.0) + val
            serie7.append(round(_ing_usd(_pm_d, moneda_v), 2))
        ...
        filas.append({
            ...
            "roas_mod": roas_mod, "ingresos_mod": imod_usd,
...
def seccion_por_pais():
    ...
    for pais_raw, d in db.ventas_por_pais(cutoff, hasta).items():
        ...
        # Cada venta se convierte con SU moneda (la del país); '?' usa el respaldo mv.
        ing_usd = sum(val * _r(mv if mon == "?" else mon)
                      for mon, val in (d.get("por_moneda") or {}).items())
        ingresos[nombre] = ingresos.get(nombre, 0.0) + ing_usd
...
def seccion_vista_general():
    ...
    if fuente != "meta" and not _filtro_acota:
        ...
        _fb_moneda = (moneda_v if (moneda_v and moneda_v != "auto")
                      else (monc.most_common(1)[0][0] if monc else "USD"))
        sa_num, sa_ing_usd = 0, 0.0
        for fnt, d in sin_por_fuente.items():
            ...
            # Cada venta sin ad_id se convierte con SU moneda (por país); las de moneda
            # desconocida ('?') usan el respaldo _fb_moneda.
            for mon, val in (d.get("por_moneda") or {}).items():
                sa_ing_usd += val * fx.tasa_a_usd(_fb_moneda if mon == "?" else mon)
        if sa_num:
            sin_adid = {"num": sa_num, "ingreso_usd": sa_ing_usd}
...
def _dialog_info():
    ...
    # Cada venta se convierte con SU moneda (país); '?' usa la moneda de la cuenta.
    _mv_fb = f.get("moneda") or "USD"
    ing_por_dia = {d: sum(val * fx.tasa_a_usd(_mv_fb if mon == "?" else mon)
                          for mon, val in (v.get("por_moneda") or {}).items())
                   for d, v in vd.items()}
...
[data-testid="stAppViewContainer"], .stApp{ background:var(--bg) !important; overflow:hidden; }
...
.login-bg{ position:fixed; inset:0; z-index:0; overflow:hidden; pointer-events:none; background:var(--bg); }
...
div[data-testid="stVerticalBlockBorderWrapper"]{
    ...
    background:var(--card) !important; backdrop-filter:none;
    border:1px solid var(--card-brd) !important; border-radius:20px !important;
...
.stTextInput div[data-baseweb="input"], .stTextInput input{ background:var(--card) !important;
    border-radius:10px !important; }
.stTextInput input{ border:1px solid var(--brd2) !important; color:#18181B !important;
...
.login-feats2 .sep{ width:1px; height:14px; background:var(--card-brd); }
...
def _inject_theme_vars():
    """Paleta (:root) según ?theme=light|dark que manda Datibot, para que el panel
    siga el tema de Datibot (claro/oscuro). Se llama ANTES que cualquier otro CSS."""
...
def _inject_css():
    ...
    /* La paleta (:root con --bg/--card/--txt/… por tema) se inyecta ANTES en
       _inject_theme_vars() según ?theme=light|dark que manda Datibot. */
    /* Contenedores TRANSPARENTES para que se vea el fondo (glow + ::before). */
    ...
    [data-testid="stSidebar"]{ background:var(--sbar) !important; border-right:1px solid var(--card-brd); }
...
def _panel_supabase():
    ...
    with st.form("form_supabase"):
        ...
        col_pais = c6.text_input("Columna país (opcional)", value=m.get(supa.K_PAIS, ""),
                                 help="Para ver la facturación por país. La moneda de "
                                      "cada venta se deriva del país (Perú→PEN, Chile→CLP, "
                                      "Venezuela→VES, Ecuador→USD, Colombia→COP…).")
        col_moneda = st.text_input(
            "Columna moneda (opcional)", value=m.get(supa.K_MONEDA, ""),
            help="Solo si tu tabla tiene una columna con el código de moneda (PEN, COP, "
                 "VES…). Si se llena, tiene prioridad sobre el país. Si la dejas vacía, la "
                 "moneda se deriva del país de cada venta.")
        gcol1, gcol2 = st.columns(2)
        probar = gcol1.form_submit_button("Probar y ver columnas")
        guardar = gcol2.form_submit_button("Guardar mapeo", type="primary")

    if guardar:
        supa.guardar_mapeo(tabla, col_ad, col_val, col_hora, col_prod, col_id, col_pais, col_moneda)
    ...
    st.divider()
    st.caption("Usa esto para borrar las ventas ya importadas de Supabase y volver a traerlas. "
               "Corrige la **fecha** (zona horaria) y ahora también guarda la **moneda de cada "
               "venta** según su país (Perú→PEN, Chile→CLP, Venezuela→VES, Ecuador→USD, "
               "Colombia→COP…), para convertir cada una a USD con su tasa correcta. "
               "**Hazlo una vez** después de esta actualización para que las ventas viejas "
               "tomen su moneda.")
    if st.button("🔁 Borrar ventas de Supabase y re-importar (corrige fechas y moneda)"):
...
def pagina_tutoriales():
    st.title("Tutoriales")
    st.caption("El tutorial está unificado dentro de Datibot, junto con el de toda la plataforma.")
    st.markdown(
        "Para no tener una gu\u00eda en cada lado, el paso a paso de **Reporte de anuncios** "
        "\u2014conectar Facebook (token de Usuario del Sistema), un 2\u00ba Business, Supabase, "
        "la atribuci\u00f3n con CAPI (`ctwa_clid`) y la facturaci\u00f3n por pa\u00eds\u2014 vive ahora en el "
        "**Tutorial de Datibot**, en la secci\u00f3n **\u00abReporte de anuncios\u00bb**."
    )
    st.link_button(
        "\U0001F4D6  Abrir el tutorial completo en Datibot",
        "https://app.datibot.lat/tutorial#reporte",
    )
    st.caption("Se abre en una pesta\u00f1a nueva, con tu misma sesi\u00f3n.")
...
def pagina_graficos():
    ...
    vdt = db.ventas_diarias_totales(desde, manana)   # {dia: {num, ingreso_nat, por_moneda}}
    # Cada venta se convierte con SU moneda (país); '?' usa el respaldo mv.
    ing_dia = {d: sum(val * _rc(mv if mon == "?" else mon)
                      for mon, val in (vdt.get(d, {}).get("por_moneda") or {}).items())
               for d in dias}
...
# --------------------------------------------------------------------------- #
def main():
    _inject_theme_vars()   # paleta según ?theme= (sigue el tema de Datibot)
    _gate_password()
    _inject_css()
